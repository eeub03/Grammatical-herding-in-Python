...
class Attractor:
    ...
    def __init__(self, type_, effect, xcor, ycor, side):
        ...
        self.side = side
        print('%s __init__ type:%s effect:%s x_min:%.3f x_max:%.3f y_min:%.3f y_max:%.3f' % (
        __name__, self.type_, self.effect, self.xcor[0], self.xcor[1], self.ycor[0], self.ycor[1]))
        ...
        if force_fields:
            for force_field in force_fields:
                self.force_fields.append(
                    Attractor(force_field['type'], force_field['effect'], force_field['x'], force_field['y'],
                              force_field['size']))
